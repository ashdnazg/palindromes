...
def is_pruned_by_table(min_decimal_palindrome, max_decimal_palindrome, lookup_table):
    ...
    shared_bit_suffix = int(shared_bit_prefix[::-1], 2)
    subtraction_suffix = (shared_bit_suffix - min_decimal_palindrome) % (1 << len(shared_bit_prefix))
...
def profile():
    decimal_length = 4
    best_max_table_digits = 1
    while True:
        max_table_digits = best_max_table_digits
        ...
        while True:
            ...
            if duration > last_duration:
                ...
                best_max_table_digits = max_table_digits
                max_table_digits += 1

        print("length: %s, max_table_digits: %s" % (decimal_length, best_max_table_digits))
        decimal_length += 1
