    ...
    F: FnMut(&T, &T) -> bool,
{
    ...
    loop {
        ...
        if left.len() < right.len() {
            recurse(left, is_less, pred, limit, cancel_marker);
            v = right;
            pred = Some(pivot);
        } else {
            recurse(right, is_less, Some(pivot), limit, cancel_marker);
            v = left;
        }
    }
}
...
pub(crate) fn quicksort<T, F>(v: &mut [T], mut is_less: F, cancel_marker: &AtomicBool)
where
    F: FnMut(&T, &T) -> bool,
{
    ...
    recurse(v, &mut is_less, None, limit, cancel_marker);
}
